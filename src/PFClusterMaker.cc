...
#include "DataSleuth/DataSleuth/interface/PFClusterMaker.h"
...
PFClusterMaker::PFClusterMaker(const edm::ParameterSet& iConfig) {
  ...
  inputTagPFClustersHF_  = iConfig.getParameter<edm::InputTag>("PFClustersHF");
  inputTagPFClustersHO_ = iConfig.getParameter<edm::InputTag>("PFClustersHO");
...
void PFClusterMaker::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  found = iEvent.getByLabel(inputTagPFClustersHF_, clustersHF);      
  
  if(!found )
    LogError("PFBlockProducer")<<" cannot get HF clusters: "
                               <<inputTagPFClustersHF_<<endl;
  
  //HFHAD PFClusters
  edm::Handle< reco::PFClusterCollection > clustersHO;
  found = iEvent.getByLabel(inputTagPFClustersHO_, clustersHO);      
  
  if(!found )
    LogError("PFBlockProducer")<<" cannot get HO clusters: "
                               <<inputTagPFClustersHO_<<endl;

  edm::Handle<reco::PFClusterMETCollection> pfClusterMet_h;
  found = iEvent.getByLabel(inputTagPFClusterMET_, pfClusterMet_h);

  if(!found )
    LogError("PFBlockProducer")<<" cannot get PF cluster MET: "
                               << inputTagPFClusterMET_ << endl;
  ...
  for (reco::PFClusterCollection::const_iterator it = clustersHO->begin(); it != clustersHO->end(); it++){
  ...
  *pfc_met = pfClusterMet_h.isValid() ? (pfClusterMet_h->front()).pt() : -9999;
  *pfc_metphi = pfClusterMet_h.isValid() ? (pfClusterMet_h->front()).phi() : -9999;  
  *pfc_sumet = pfClusterMet_h.isValid() ? (pfClusterMet_h->front()).sumEt() : -9999;
